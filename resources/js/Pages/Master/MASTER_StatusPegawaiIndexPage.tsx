...
export default function MASTER_StatusPegawaiIndexPage({ auth, pagination }: PageProps<{
    pagination: PaginationData<StatusPegawais>;
}>) {
    ...
    const handleFormSubmit = (event: SyntheticEvent<HTMLFormElement>) => {
        ...
        const zodStatusPegawaiResult = statusPegawaiSchema.safeParse({
            nama: nama,
            keterangan: keterangan
        });
        if (!zodStatusPegawaiResult.success) {
            const errorMessages = zodStatusPegawaiResult.error.issues[0].message;
        ...
        axios.post(route('status-pegawai.create'), {
            ...
            .then(() => {
                notifyToast('success', 'Status Pegawai berhasil ditambahkan!');
    ...
    const handleDeleteStatusPegawai = () => {
        setOnSubmitDelete(true);
        axios.post(route('status-pegawai.delete'), {
            id: deleteDialog.statusPegawaiId,
        })
            .then(() => {
                notifyToast('success', 'Status Pegawai terpilih berhasil dihapus!');
                                                    ...
                                                    <td className={ classes }>
                                                        <div className="w-32 flex gap-2.5 items-center justify-start">
                                                            <Tooltip content="Detail">
                                                                <Link
                                                                    href={ route('master.status-pegawai.details', { q: pagination.data[index].id }) }>
                        ...
                        <div className="p-3 border border-gray-300 rounded bg-gray-100">
                            <Typography variant="h6" className="text-gray-900 truncate">
                                ...
                                <span className="font-bold">
                                    { pagination.data.find((statusPegawai) => statusPegawai.id === deleteDialog.statusPegawaiId)?.nama ?? '-' }
                                </span>
                            </Typography>
                            <Typography variant="h6" className="text-gray-900">
                                Semua Pegawai yang terdaftar dalam Status Pegawai ini akan kehilangan Status Pegawainya
                            </Typography>
                        </div>
                    </DialogBody>
                    <DialogFooter>
                        ...
                        <Button color="red" onClick={ handleDeleteStatusPegawai } loading={onSubmitDelete} className="flex justify-items-center">
                            <span>Hapus</span>
                        </Button>
                    </DialogFooter>
                </Dialog>
