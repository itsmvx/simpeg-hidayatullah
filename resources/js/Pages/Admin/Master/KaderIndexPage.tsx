...
export default function MasterManageKaderPage({ auth, kaders }: PageProps<{
    kaders: {
        id: string;
        nama: string;
        keterangan: string;
    ...
    const adjustData = useMemo(() => {
        ...
        return kaders.slice(startIndex, lastIndex);
    }, [ kaders, viewPerPage ]);
    ...
    const nextPage = () => {
        const totalPages = Math.ceil(kaders.length / viewPerPage);
    ...
    const handleFormSubmit = (event: SyntheticEvent<HTMLFormElement>) => {
        ...
        const kaderschema = z.object({
            nama: z.string().min(1, { message: "Nama Kader tidak boleh kosong" }),
            keterangan: z.string().min(1, { message: "Keterangan Kader tidak boleh kosong" })
        });
        const zodUnitResult = kaderschema.safeParse({
            nama: nama,
        ...
        axios.post(route('kader.create'), {
            ...
            .then(() => {
                notifyToast('success', 'Kader berhasil ditambahkan!', theme as 'light' | 'dark');
                router.reload({ only: ['kaders'] });
            })
            .catch((err: unknown) => {
    ...
    const handleDeleteSubmit = () => {
        ...
        const kaderschema = z.object({
            id: z.string().min(1, { message: "Kader belum terpilih" }),
        });
        const zodUnitResult = kaderschema.safeParse({
            id: id,
        });
        ...
        axios.post(route('kader.delete'), {
            id: id,
        })
            .then(() => {
                notifyToast('success', 'Kader berhasil dihapus!', theme as 'light' | 'dark');
                router.reload({ only: ['kaders'] });
            })
            .catch((err: unknown) => {
                const errMsg: string = err instanceof AxiosError
    ...
    useEffect(() => {
        if (search.length < 1) {
            ...
            setCurrPage(1);
            const matchkaders = kaders.filter((kader) => kader.nama.toLowerCase().includes(search.toLowerCase()));
            setData(matchkaders);
        }
    }, [ search ]);
                                                            ...
                                                            <IconButton
                                                                ...
                                                                onClick={() => {
                                                                    setDeleteDialog((prevState) => ({
                                                                        ...
                                                                        open: true,
                                                                        id: id,
                                                                        nama: kaders.find((kader) => kader.id === id)?.nama ?? '-'
                                                                    }))
                                                                }}
                                                            >
                ...
                <Dialog open={deleteDialog.open} handler={handleOpenDelete}>
                    ...
                    <DialogBody>
                        ...
                        <p className="text-sm text-gray-900 font-medium">
                            ...
                            Pegawai yang terhubung akan akan kehilangan status kader
                        </p>
                    </DialogBody>
                    <DialogFooter>
                        <Button
