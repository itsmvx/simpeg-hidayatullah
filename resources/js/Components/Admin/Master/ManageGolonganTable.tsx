...
const TABLE_HEAD = ["Golongan", "Keterangan", "Tanggal dibuat", ""];

export const ManageGolonganTable = ({ golongansData }: {
    golongansData: {
        id: string;
        nama: string;
        keterangan: string;
    ...
    const [ openCreateGolongan, setOpenCreateGolongan ] = useState(false);
    ...
    const adjustData = (currpage: number, viewperpage: number) => {
        ...
        return golongansData.slice(startIndex, lastIndex);
    };
    ...
    const nextPage = () => {
        const totalPages = Math.ceil(golongansData.length / viewPerPage);
    ...
    const handleOpen = () => setOpenCreateGolongan(true);
    const formInputInit = {
        nama: '',
        keterangan: ''
    };
            ...
            <Card className="h-full w-full">
                <CardHeader floated={false} shadow={false} className="rounded-none">
                    <div className="mb-8 flex items-center justify-between gap-x-3">
                        ...
                        <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                            <Button
                                onClick={() => setOpenCreateGolongan(true)}
                    ...
                    <table className="mt-4 w-full min-w-max table-auto text-left">
                        ...
                        {data.map(
                            ({ id, nama, keterangan, created_at }, index) => {
                                const isLast = index === golongansData.length - 1;
                ...
                <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
                    <Typography variant="small" color="blue-gray" className="font-normal">
                        Halaman { currPage } dari { Math.ceil(golongansData.length / viewPerPage) }
                    </Typography>
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            {
                                Array.from({ length: Math.ceil(golongansData.length / viewPerPage) }).map((_, index) => (
                                    <IconButton
                                        key={index}
                        ...
                        <Button
                            ...
                            className="flex items-center gap-2 rounded-full"
                            onClick={ nextPage }
                            disabled={ currPage === Math.ceil(golongansData.length / viewPerPage) }
                        >
                            Next
                    ...
                    <Card>
                        ...
                        <CardFooter className="pt-0 flex gap-3 justify-between">
                            <Button
                                color="red"
                                variant="text"
                                onClick={() => setOpenCreateGolongan(false)}
                                fullWidth
                            >
                                Batal
                            </Button>
